...
public abstract class Learner {
    ...
    public Document createDocument(Field[] fields, String label) {
        ...
        MultiFieldDocument doc = new MultiFieldDocument(fields, this.lexicon, true);
    ...
    protected abstract void internal_generateVector(TrainingCorpus documents) throws Exception;
    ...
    public void generateVectorFile(TrainingCorpus corpus) throws Exception {
        ...
        if (this.lexicon.getLabelNum() < 2) {
            throw new Exception("There must be at least two different class values in the training corpus");
        }
        ...
        if (this.lexicon.getLabelNum() > 2) {
            if (supportsMultiLabels() == false)
                throw new Exception("Leaner implementation does not support multiple classes");
        }
        ...
        if (keepNBestAttributes != -1) {
            ...
            AttributeScorer scorer = logLikelihoodAttributeScorer.getScorer(corpus, lexicon);
     ...
     * 
     * @param overwrite deletes any existing data in the model directory
     * @return an instance of a Learner corresponding to the implementationName
     * @throws ClassNotFoundException
     * @throws IllegalAccessException
     * @throws InstantiationException
     */
    public static Learner getLearner(String workdirectory, String implementationName, boolean overwrite)
        throws Exception {
        ...
        String model_file_name = workdirectory + File.separator + Parameters.modelName;
        String lexicon_file_name = workdirectory + File.separator + Parameters.lexiconName;
        String vector_file_name = workdirectory + File.separator + Parameters.vectorName;
        String raw_file_name = workdirectory + File.separator + Parameters.rawName;
        ...
        if (LibSVMModelCreator.equals(implementationName))
            learner = new LibSVMModelCreator(lexicon_file_name, model_file_name, vector_file_name);
        else if (LibLinearModelCreator.equals(implementationName))
            learner = new LibLinearModelCreator(lexicon_file_name, model_file_name, vector_file_name);
        else
